...
export default function Tabla({
  ...
  const [selectedCellcut, setSelectedCellcut] = useState<{
    key: string;
    status: string;
  } | null>(null);
      ...
      if (result.isConfirmed) {
        ...
        try {
          const response = await fetch(
            `${API_URL}/booking/change/${bookingid}`,
            {
              method: "PUT",
  ...
  const handleCellClick = (
    ...
    if (
      selectedDay &&
      selectedDay.booking_details &&
      (selectedDay.status === "reservado" || selectedDay.status === "en espera")
    ) {
      setModalDataE({
        timeStart,
        timeEnd,
    ...
    } else if (selectedDay && selectedDay.status === "disponible") {
      setModalData({
        timeStart,
        timeEnd,
        day: days[contador],
      ...
      const updatedReservations = prev.map((reservation) => {
        if (reservation.hour_range?.start === formattedStartTime) {
          const newDays = reservation.days.map((d) => {
            if (
              format(new Date(d.day_name), "yyyy-MM-dd") ===
      ...
      <Table className="table-fixed w-full text-sm border-separate min-w-[1500px]">
        <thead className="bg-gray-100">
          ...
          <tr>
            ...
            {days.map((day, index) => (
              <th
                key={index}
                className="border px-4 py-2 text-xs bg-[#8D9EC1] text-white"
              >
        ...
        <tbody>
          {reservations.map((reservation, index) => (
            <tr key={index}>
              <td className="rotate-0 border text-[12px] font-bold font-inter text-center bg-white text-[#454545]">
                CAMPO {field}
              </td>
              ...
              {reservation.days.map((day, dayIndex) => {
                ...
                return (
                  <td
                    key={dayIndex}
                    onContextMenu={(e) =>
                      handleContextMenu(e, cellKey, status, idbooking, iduser)
                    } // Convertir a string
                    onTouchEnd={handleTouchEnd} // Aquí cancelamos si suelta antes de 2 segundos
                    onTouchStart={(e) =>
                      handleTouchStart(e, cellKey, status, idbooking, iduser)
                    }
                    onClick={() => {
                      ...
                      if (band == true) {
                        console.log(
                          "Ejecutando handleCutClick para:",
                          bookingid,
                          "en celda:",
                          cellKey
                        );
                        ...
                        const selectedDay = reservation.days.find(
                          (d) => d.day_name === day.day_name
                        );
                        if (selectedDay) {
                          console.log(
                            "Ejecutando handleCellClick, abriendo modal"
                          );
                          handleCellClick(
                            reservation.hour_range.start,
                            reservation.hour_range.end,
